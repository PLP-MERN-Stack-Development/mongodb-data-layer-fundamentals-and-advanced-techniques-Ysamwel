// insert_books.js
// Script to populate MongoDB with sample book data

require('dotenv').config();
const { MongoClient } = require('mongodb');

// Load MongoDB URI from .env
const uri = "mongodb+srv://yvonneatieno2000_db_user:012345@cluster0.oeeybsm.mongodb.net/plp_bookstore?retryWrites=true&w=majority";
;
...
async function insertBooks() {
  ...
  try {
    ...
    if (count > 0) {
      console.log(`Collection already has ${count} docs. Dropping...`);
      await collection.drop();
      console.log('Collection dropped successfully.');
    }

    const result = await collection.insertMany(books);
    console.log(`Inserted ${result.insertedCount} books into the database!`);
  } catch (error) {
